...
describe('Broadcasts', () => {
  ...
  it('visits the dashboard', () => {
    cy.seed()
    cy.contains('Most recent batch sent on Wed Jan 31, 9:30 PM GMT+7')
    cy.contains('Total conversation starters sent: 120')
    cy.contains('Delivered successfully: 230')
    cy.contains('Failed to deliver: 2')

    cy.contains('Next batch scheduled to send on Sun Oct 10, 11:16 PM GMT+7')
  ...
  it('edit first message of far future', () => {
    ...
    cy.get('.data-edit-first-message').click()
    ...
    cy.intercept('PATCH', `${BROADCAST_PATH}/35`).as('updateBroadcast')
  ...
  it('edit first message of less than 90 minutes', () => {
    ...
    cy.get('.data-edit-second-message').click()
    cy.contains("The next batch is scheduled to send less than 90 minutes from now. Making these message updates will delay today's batch by 2-3 hours, sending at approximately")
    cy.contains('Note: second messages are sent 10 minutes after the conversation starter, only if the recipient does not reply to the starter message.')
    cy.contains('button', 'Save changes and delay the next batch').should('be.disabled')
  })
})
