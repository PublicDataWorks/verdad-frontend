...
CI (`.github/workflows/ci.yml`) runs typecheck, lint, test and build on every PR and push to `main`.
...
- RPC results are only as typed as the `T` you pass to `rpc<T>()`; nothing validates them. Generating Supabase
  types (`supabase gen types typescript`) and passing `Database` to `createClient` is the intended follow-up.
- `src/types/snippet.ts` lags the SQL (e.g. `confidence_scores.level` was read but untyped); check the RPC in
  the backend repo before trusting a field.
