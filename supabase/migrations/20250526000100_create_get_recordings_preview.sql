...
-- Create function for fetching recording previews with cursor-based pagination
...
BEGIN
    ...
    SELECT
        ...
        s1.id AS stage_1_id,
        -- Get primary language from first snippet if available
        COALESCE(
    ...
    WHERE
        ...
        -- Label filter (recordings that have snippets with this label)
        AND (
        ...
        -- Language filter (based on snippets' detected language)
        AND (
    ...
    -- Get the cursor for the next page (recorded_at of the last item we're returning)
...
GRANT EXECUTE ON FUNCTION get_recordings_preview TO anon;
GRANT EXECUTE ON FUNCTION get_recordings_preview TO authenticated;
